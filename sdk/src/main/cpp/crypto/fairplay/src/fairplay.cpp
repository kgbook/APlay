/**
 *  Copyright (C) 2026  kgbook
 *
 *  This library is free software; you can redistribute it and/or
 *  modify it under the terms of the GNU Lesser General Public
 *  License as published by the Free Software Foundation; either
 *  version 2.1 of the License, or (at your option) any later version.
 */

#include "fairplay.hpp"

#include <cstddef>

namespace aplay {
namespace crypto {
namespace fairplay {

static const std::uint8_t kSetupReplies[4][142] = {
    {},
    {},
    {},
    {}};

static const std::uint8_t kHandshakeHeader[12] = {
    0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14};

bool setup_response(const std::vector<std::uint8_t>& request,
                    std::vector<std::uint8_t>& response) {
    response.clear();
    if (request.size() != 16 || request[4] != 0x03 || request[14] > 3) {
        return false;
    }

    const std::uint8_t* begin = kSetupReplies[request[14]];
    response.assign(begin, begin + 142);
    return true;
}

bool handshake_response(const std::vector<std::uint8_t>& request,
                        std::vector<std::uint8_t>& response) {
    response.clear();
    if (request.size() != 164 || request[4] != 0x03) {
        return false;
    }

    response.assign(kHandshakeHeader, kHandshakeHeader + sizeof(kHandshakeHeader));
    response.insert(response.end(), request.begin() + 144, request.begin() + 164);
    return true;
}

} // namespace fairplay
} // namespace crypto
} // namespace aplay
