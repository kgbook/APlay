...
- `aplay_sdk` 可构建并输出 `.so`。
- `aplay_jni` 可作为 Java SDK native binding 构建。
- `aplay_napi` 保留 ETS SDK native binding 条件编译入口，并由 Harmony HAR native 构建启用。
- `:aplay-sdk` 可构建 Java SDK 并输出 Android AAR。
- `sdk/src/main/ets` 提供 ETS SDK/Harmony HAR facade。
- `app/android` host stub 可通过 Java SDK 构建。
- 后续扩展为端口监听、请求响应、退出清理。
...
- `sdk` C++ SDK 对象库与 `aplay_sdk` `.so` 可编译入口。
- `sdk` Java SDK AAR 可编译入口。
- `sdk` ETS SDK/Harmony HAR module 入口。
