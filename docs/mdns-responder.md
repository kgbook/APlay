...
The behavior follows the UxPlay implementation in `/Volumes/workspace/me/opensource/UxPlay/lib/mdnsd.c` and the packet analysis notes in `/Volumes/workspace/me/blog/content/articles/Android/airplay_mdns_discovery/index.md`.

The local capture fixture is `resources/pcap/Airplay_Discovery_and_Connection.pcapng`. It contains AirPlay and RAOP discovery traffic for `UxPlay@iMac._airplay._tcp.local` and `B246EE395763@UxPlay@iMac._raop._tcp.local`.
...
This matches the engineering tradeoff from the UxPlay implementation: simple packet code first, with a conservative size threshold.
...
The IPv4 value is stored as a big-endian numeric address. `core::network::parse_ipv4_address("192.168.1.101", address)` fills the expected value for packet encoding.
...
- Examples may set fixture-specific profile fields, but they must not hand-build AirPlay or RAOP TXT records.

## Example Coverage

`example/mdns_replay.cpp` validates the responder offline:
...
- Scans `resources/pcap/Airplay_Discovery_and_Connection.pcapng` for the DNS-encoded AirPlay/RAOP names used by the capture.
...
Add `--serve` to attempt live multicast on UDP 5353. The default mode generates and parses announcement packets offline so the example remains runnable when the host operating system already owns the mDNS port.
