...
- `sdk` 是共享 SDK module。
- `sdk/src/main/cpp` 提供 C++ SDK 对象源码，并在 Linux 输出共享的 `APlaySdk` `.so`。
- `sdk/src/main/cpp/utils` 提供跨平台工具辅助实现，例如 socket/thread/poll。
- `sdk/src/main/cpp/third-party` 存放 SDK 使用的第三方 C/C++ 依赖子模块。
- `sdk/src/main/cpp/osal/android/jni` 提供 Java SDK native binding，并为 Android APK 打包输出 `libAPlaySdk.so`。
- `sdk/src/main/cpp/osal/harmony/napi` 提供 ETS SDK native binding，对外输出 `libaplay_napi.so`。
- `sdk/src/main/java` 提供 Java SDK，对外输出 Android `APlaySdk` AAR。
...
- `app/android` 是 Android Gradle 入口，对外显示 `APlayReceiver` Gradle root，通过 `:APlaySdk` module 使用 Java SDK，并输出 `APlayReceiver` APK。
